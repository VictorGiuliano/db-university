...
SELECT `students`.`surname`,`students`.`name`, `degrees`.`name`AS 'Corso' FROM `degrees` JOIN `students` ON `degrees`.`id` = `students`.`degree_id` WHERE `degrees`.`name` = 'Corso di Laurea in Economia';
